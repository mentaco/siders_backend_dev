...
def get_message(user_id):
    ...
    CASE
        ...
        WHEN m.message_from = %s AND LEFT(m.message_to, 1) = 'C' THEN c_to_info.company_logomark
        WHEN LEFT(m.message_from, 1) = 'S' AND m.message_to = %s THEN s_handle.profile_image
        WHEN LEFT(m.message_from, 1) = 'C' AND m.message_to = %s THEN c_info.company_logomark
...
def read_message():
    try:
        ...
        message_from = data.get('message_from')
        message_to = data.get('message_to')
        params = (message_from,message_to)
        query = 'UPDATE message_t SET message_status_code = 1 WHERE message_from = %s AND message_to = %s'
        exec_query(query, params, fetch_all=False)
        return jsonify({'message': 'Data added successfully'})
    except Exception as e:
        return jsonify({'error': str(e)}), 400
